...
def sort_df(my_df):
    ...
    my_df = my_df.sort_values(by='properties.TYPE_SITE_INTERVENTION').reset_index(drop = True)
    return my_df
...
def get_neighborhoods(montreal_data):
    ...
    mtl_df = pd.json_normalize(montreal_data, 'features')
    locations = mtl_df['properties.NOM'].unique()
    return locations
